...
#  include <pareto_front/boost_subset/math/complex/details.hpp>
#endif
...
std::complex<T> acos(const std::complex<T>& z)
{
   ...
   // note that we have tweaked a_crossover as per: https://svn.boost.org/trac/boost_subset/ticket/7290
   //
   ...
   else
   {
      ...
      else
      {
         ...
         if(y <= (std::numeric_limits<T>::epsilon() * std::fabs(xm1)))
         {
            ...
            {
               // This deviates from Hull et al's paper as per https://svn.boost.org/trac/boost_subset/ticket/7290
