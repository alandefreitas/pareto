...
#include <pareto_front/boost_subset/swap.hpp>
#include <pareto_front/boost_subset/integer.hpp>

#include <pareto_front/boost_subset/mpl/assert.hpp>

#include <pareto_front/boost_subset/type_traits/is_unsigned.hpp>
#include <pareto_front/boost_subset/type_traits/alignment_of.hpp>
#include <pareto_front/boost_subset/type_traits/aligned_storage.hpp>
...
#include <pareto_front/boost_subset/iterator/iterator_concepts.hpp>

#include <pareto_front/geometry/index/detail/assert.hpp>
#include <pareto_front/geometry/index/detail/exception.hpp>

#include <pareto_front/geometry/index/detail/varray_detail.hpp>
...
struct checker
{
    ...
    static inline void check_capacity([[maybe_unused]] Varray const& v, [[maybe_unused]] size_type s)
    {
    ...
    static inline void throw_out_of_bounds([[maybe_unused]] Varray const& v, [[maybe_unused]] size_type i)
    {
        if ( v.size() <= i )
    ...
    static inline void check_index([[maybe_unused]] Varray const& v, [[maybe_unused]] size_type i)
    {
    ...
    static inline void check_not_empty([[maybe_unused]] Varray const& v)
    {
    ...
    static inline void check_iterator_end_neq([[maybe_unused]] Varray const& v, [[maybe_unused]] const_iterator position)
    {
    ...
    static inline void check_iterator_end_eq([[maybe_unused]] Varray const& v, [[maybe_unused]] const_iterator position)
    {
        BOOST_GEOMETRY_INDEX_ASSERT(v.begin() <= position && position <= v.end(), "iterator out of bounds");
...
private:
    ...
    void swap_dispatch_impl(iterator first_sm, iterator last_sm, iterator first_la, iterator last_la, boost::true_type const& /*use_memop*/)
    {
        ...
        for (; first_sm != last_sm ; ++first_sm, ++first_la)
        {
            ...
            ::memcpy(temp_ptr, std::addressof(*first_sm), sizeof(value_type));
            ::memcpy(std::addressof(*first_sm), std::addressof(*first_la), sizeof(value_type));
            ::memcpy(std::addressof(*first_la), temp_ptr, sizeof(value_type));
        }
...
public:
    ...
    // nothrow
    Value * data() { return std::addressof(*(this->ptr())); }
    const Value * data() const { return std::addressof(*(this->ptr())); }
