...
#include <pareto_front/boost_subset/mpl/or.hpp>
#include <pareto_front/boost_subset/mpl/int.hpp>

#include <pareto_front/boost_subset/type_traits/is_same.hpp>
#include <pareto_front/boost_subset/type_traits/remove_const.hpp>
#include <pareto_front/boost_subset/type_traits/remove_reference.hpp>
#include <pareto_front/boost_subset/type_traits/has_trivial_assign.hpp>
...
#include <pareto_front/boost_subset/type_traits/has_trivial_move_constructor.hpp>
#include <pareto_front/boost_subset/type_traits/has_trivial_move_assign.hpp>
...
//#include <pareto_front/boost_subset/type_traits/has_nothrow_assign.hpp>
//#include <pareto_front/boost_subset/type_traits/has_nothrow_destructor.hpp>
...
#include <memory> // #include <pareto_front/boost_subset/core/addressof.hpp>
                       ...
                       boost::mpl::bool_<true> const& /*use_memmove*/)
{
    ...
    ::memmove(std::addressof(*dst), std::addressof(*first), sizeof(value_type) * d);
    return dst + d;
}
                              ...
                              boost::mpl::bool_<true> const& /*use_memcpy*/)
{
    ...
    ::memcpy(std::addressof(*dst), std::addressof(*first), sizeof(value_type) * d);
    return dst + d;
}
                              ...
                              boost::mpl::bool_<false> const& /*use_memcpy*/)
{
    ...
    BOOST_TRY
    {
        ...
        for (; first != last; ++first, ++o )
            new (std::addressof(*o)) value_type(boost::move(*first));
    }
    BOOST_CATCH(...)
    {
        destroy(dst, o);
                           ...
                           boost::mpl::bool_<true> const& /*use_memmove*/)
{
    ...
    BDO foo(dst - d);
    ::memmove(std::addressof(*foo), std::addressof(*first), sizeof(value_type) * d);
    return foo;
}
                                 ...
                                 DisableTrivialInit const& /*not_used*/)
{
    ...
    {
        for ( ; it != last ; ++it )
            new (std::addressof(*it)) value_type();                           // may throw
    }
    BOOST_CATCH(...)
    {
        destroy(first, it);
...
{
    typedef typename ::boost::iterator_value<I>::type value_type;
    new (static_cast<void*>(::std::addressof(*pos))) value_type();                      // may throw
}
...
void construct_copy_dispatch(I pos, V const& v,
                             boost::mpl::bool_<true> const& /*use_memcpy*/)
{
    ::memcpy(std::addressof(*pos), std::addressof(v), sizeof(V));
}
                             ...
                             boost::mpl::bool_<false> const& /*use_memcpy*/)
{
    ...
    new (static_cast<void*>(std::addressof(*pos))) V(p);                      // may throw
}
...
{
    typedef typename boost::iterator_value<I>::type V;
    new (static_cast<void*>(std::addressof(*pos))) V(::boost::move(p));       // may throw
}
...
{
    typedef typename boost::iterator_value<I>::type V;
    new (static_cast<void*>(std::addressof(*pos))) V(::boost::forward<Args>(args)...);    // may throw
}
...
{                                                                                                   \
    typedef typename boost::iterator_value<I>::type V;                                              \
    new                                                                                             \
    (static_cast<void*>(std::addressof(*pos)))                                                    \
