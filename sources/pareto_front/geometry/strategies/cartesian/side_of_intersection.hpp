...
#include <pareto_front/boost_subset/math/common_factor_ct.hpp>
#include <pareto_front/boost_subset/math/common_factor_rt.hpp>
#include <pareto_front/boost_subset/multiprecision/cpp_int.hpp>
#endif
...
public :
                ...
                Segment const& c, Point const& fallback_point)
    {
...
#ifdef BOOST_GEOMETRY_SIDE_OF_INTERSECTION_DEBUG
        [[maybe_unused]] T const result1 = geometry::detail::determinant<T>
                    (
                        dx_c * d,                   dy_c * d,
        ...
        [[maybe_unused]] T const result2 = d * geometry::detail::determinant<T>
                    (
                        dx_c,                   dy_c,
                        d * (ax - cx) + dx_a * da,  d * (ay - cy) + dy_a * da
                    );
        // Write out:
        [[maybe_unused]] T const result3 = d * (dx_c * (d * (ay - cy) + dy_a * da)
                             - dy_c * (d * (ax - cx) + dx_a * da));
        // Write out in braces:
        [[maybe_unused]] T const result4 = d * (dx_c * d * (ay - cy) + dx_c * dy_a * da
                             - dy_c * d * (ax - cx) - dy_c * dx_a * da);
        // Write in terms of d * XX + da * YY
        [[maybe_unused]] T const result5 = d * (d * (dx_c * (ay - cy) - dy_c * (ax - cx))
