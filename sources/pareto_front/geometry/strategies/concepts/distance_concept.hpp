...
private :
    ...
    struct checker
    {
        ...
        static void apply(ApplyMethod)
        {
            ...
            [[maybe_unused]] typedef typename strategy::distance::services::tag
                <
                    Strategy
                >::type tag;
            ...
            [[maybe_unused]] Strategy* str = 0;
            ptype1 *p1 = 0;
            ptype2 *p2 = 0;
            [[maybe_unused]] rtype r = str->apply(*p1, *p2);

            // 6) must define (meta)struct "get_comparable" with apply
            [[maybe_unused]] ctype c = strategy::distance::services::get_comparable
                <
                    Strategy
                >::apply(*str);
...
private :
    ...
    struct checker
    {
        ...
        static void apply(ApplyMethod)
        {
            ...
            [[maybe_unused]] rtype r = str->apply(*p, *sp1, *sp2);

            // 6) must define (meta-)struct "get_comparable" with apply
            [[maybe_unused]] ctype cstrategy = services::get_comparable<Strategy>::apply(*str);
